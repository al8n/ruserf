    ...
    T: Transport,
  {
    ...
    if resp.ack() {
      ...
      if c.responses.contains(&resp.from) {
        tracing::error!("debug: local {} handle query response {} response {:?} exit", _local, resp.id, resp.from);
        #[cfg(feature = "metrics")]
        {
      ...
      tracing::error!("debug: local {} handle query response {} response {:?}", _local, resp.id, resp.from);
      if let Err(e) = self
