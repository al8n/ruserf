...
pub(crate) struct CrateEvent<T, D>(
  pub(crate) Either<CrateEventKind<T, D>, Arc<CrateEventKind<T, D>>>,
)
where
  ...
  T: Transport,
{
  ...
  pub fn ty(&self) -> EventType {
    match self.kind() {
      CrateEventKind::Member(e) => EventType::Member(e.ty),
      CrateEventKind::User(_) => EventType::User,
      CrateEventKind::Query(_) => EventType::Query,
      CrateEventKind::InternalQuery { .. } => EventType::InternalQuery,
    }
  }
