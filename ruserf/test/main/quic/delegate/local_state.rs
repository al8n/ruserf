  ...
  ($rt:ident) => {
    paste::paste! {
      mod [< $rt:snake >] {
        ...
        use ruserf_core::tests::{delegate::delegate_local_state, next_socket_addr_v4, next_socket_addr_v6};
        use smol_str::SmolStr;

        #[test]
        fn test_delegate_local_state_v4() {
          [< $rt:snake _run >](async move {
            let name = "delegate_local_state1_v4";
            ...
            let name = "delegate_local_state2_v4";
        ...
        fn test_delegate_local_state_v6() {
          [< $rt:snake _run >](async move {
            let name = "delegate_local_state1_v6";
            let mut opts = QuicTransportOptions::with_stream_layer_options(SmolStr::new(name), quinn_stream_layer::<[< $rt:camel Runtime >]>().await);
            opts.add_bind_address(next_socket_addr_v6());

            let name = "delegate_local_state2_v6";
            let mut opts2 = QuicTransportOptions::with_stream_layer_options(SmolStr::new(name), quinn_stream_layer::<[< $rt:camel Runtime >]>().await);
