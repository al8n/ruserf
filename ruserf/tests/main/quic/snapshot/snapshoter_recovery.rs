      ...
      mod [< $rt:snake >] {
        ...
        fn test_serf_snapshot_recovery_v4() {
          [< $rt:snake _run >](async move {
            ...
            let mut opts = QuicTransportOptions::with_stream_layer_options(SmolStr::new(name), quinn_stream_layer_with_connect_timeout::<[< $rt:camel Runtime >]>(Duration::from_millis(100)).await);
            opts.add_bind_address(next_socket_addr_v4(0));

            let name = "serf_snapshot_recovery2_v4";
            let mut opts2 = QuicTransportOptions::with_stream_layer_options(SmolStr::new(name), quinn_stream_layer_with_connect_timeout::<[< $rt:camel Runtime >]>(Duration::from_millis(100)).await);
